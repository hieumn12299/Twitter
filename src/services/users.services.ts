...
class UsersService {
  ...
  private signEmailVerifyToken(user_id: string) {
    return signToken({
      payload: { user_id, token_type: TokenType.EmailVerifyToken, verify: UserVerifyStatus.Unverified },
  ...
  async refreshToken({
    ...
    const { iat, exp: current_exp } = await this.decodeRefreshToken(refresh_token);
    await databaseService.refreshTokens.insertOne(
      new RefreshToken({ user_id: new ObjectId(user_id), token: refresh_token, iat, exp: current_exp })
    );
    return { access_token, refresh_token };
  }
  ...
  async forgotPassword({ user_id, verify }: { user_id: string; verify: UserVerifyStatus }) {
    ...
    // Client sẽ gửi email cho người dùng để click reset password
    return {
